...
export class AppointmentItemComponent implements OnInit {
  ...
  begin_works(id_service: string){
    const id_user: string = localStorage.getItem('userId') || '';

    from(this.appointment.services).pipe(
      concatMap((service: any) => {
        const begin = id_service === service._id;
      ...
      complete: () => {
        this.refreshWorksAndCheckCompletion(); // Call once all requests are done
      },
      error: (err) => {
  ...
  begin_service_work(id_service: string, begin: boolean) {
    const id_user = localStorage.getItem('userId') || '';
    const existingWork = this.works.find(work => work.id_service === id_service);

    if (existingWork) {
      const updateData = {
        status: 'ongoing',
        datetime_service_begin: Date.now()
      };
      // Update existing work (example: toggle `begin` status)
      this.worksService.updateWork(existingWork._id, updateData).subscribe({
        next: () => this.refreshWorksAndCheckCompletion(),
        error: (err) => console.error('Update failed:', err),
      });
    } else {
      // Create new work
      this.worksService.createWork(
        id_user,
        this.appointment._id,
        id_service,
        begin
