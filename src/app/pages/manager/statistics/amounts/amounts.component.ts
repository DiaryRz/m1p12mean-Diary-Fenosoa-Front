...
export class AmountsChartComponent implements OnInit {
  ...
  private fetchData() {
    this.statisticsService.getMonthlyRevenues(this.year ,this.month).pipe(
      tap({
        next: (data: any) => {
  ...
  private updateChart() {
    ...
    this.lineChartData.datasets[0].data = filteredData.map(item => item.total_payed);
