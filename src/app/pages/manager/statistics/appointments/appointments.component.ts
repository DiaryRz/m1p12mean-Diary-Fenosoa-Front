...
export class AppointmentsChartComponent implements OnInit {
  ...
  public barChartOptions: ChartConfiguration<'bar'>['options'] = {
    ...
    plugins: {
      title: {
        display: true,
        text: `Rendez-vous mensuel (${this.year})`,
        font: {
          size: 16
        }
      },
  ...
  private fetchData() {
    this.statisticsService.getMonthlyAppointments(this.year , this.month).pipe(
      tap({
        next: (data: any) => {
  ...
  private updateChart() {
    ...
    this.barChartData.labels = filteredData.map(item => this.monthNames[item._id.month - 1]);
    this.barChartData.datasets[0].data = filteredData.map(item => item.total_confirmed_or_paid);
